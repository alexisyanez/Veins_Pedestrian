...
void TraCIMobility::nextPositionPed(const Coord& position, std::string road_id, double speed, Heading heading)
{
    EV_DEBUG << "nextPositionPed " << position.x << " " << position.y << " " << road_id << " " << speed << " " << heading << std::endl;
    isPreInitialized = false;
    this->roadPosition = position;
    this->road_id = road_id;
    this->speed = speed;
