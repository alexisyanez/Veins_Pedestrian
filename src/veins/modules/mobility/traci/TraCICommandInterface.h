...
public:
    ...
    // Code by Alexis Yáñez adding Pedestrian

    bool addPed(std::string pedId, std::string pedTypeId, std::string routeId, simtime_t emitTime_st = DEPART_TIME_TRIGGERED, double emitPosition = DEPART_POSITION_BASE, double emitSpeed = DEPART_SPEED_MAX, int8_t emitLane = DEPART_LANE_BEST);
    class VEINS_API Pedestrian {
    public:
        Pedestrian(TraCICommandInterface* traci, std::string nodeId)
            : traci(traci)
            , nodeId(nodeId)
        {
            connection = &traci->connection;
    ...
    Pedestrian pedestrian(std::string nodeId)
    {
        return Pedestrian(this, nodeId);
    }

    // End code by Alexis Y.
