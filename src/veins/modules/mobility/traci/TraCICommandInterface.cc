...
// Code by Alexis Yáñez adding pedestrian methods
...
bool TraCICommandInterface::addPed(std::string pedId, std::string pedTypeId, std::string routeId, simtime_t emitTime_st, double emitPosition, double emitSpeed, int8_t emitLane)
{
    ...
    TraCIBuffer buf = connection.query(CMD_SET_PERSON_VARIABLE, TraCIBuffer() << variableId << pedId << variableType << count << (uint8_t) TYPE_STRING << pedTypeId << (uint8_t) TYPE_STRING << routeId << (uint8_t) TYPE_INTEGER << emitTime << (uint8_t) TYPE_DOUBLE << emitPosition << (uint8_t) TYPE_DOUBLE << emitSpeed << (uint8_t) TYPE_BYTE << emitLane, &result);
    ASSERT(buf.eof());
...
void TraCICommandInterface::Pedestrian::setSpeed(double speed)
{
    uint8_t variableId = VAR_SPEED;
    uint8_t variableType = TYPE_DOUBLE;
    TraCIBuffer buf = traci->connection.query(CMD_SET_PERSON_VARIABLE, TraCIBuffer() << variableId << nodeId << variableType << speed);
    ASSERT(buf.eof());
}

int32_t TraCICommandInterface::Pedestrian::getLaneIndex() {
    return traci->genericGetInt(CMD_GET_PERSON_VARIABLE, nodeId, VAR_LANE_INDEX, RESPONSE_GET_PERSON_VARIABLE);
}

std::string TraCICommandInterface::Pedestrian::getRoadId() {
    return traci->genericGetString(CMD_GET_PERSON_VARIABLE, nodeId, VAR_ROAD_ID, RESPONSE_GET_PERSON_VARIABLE);
    }


std::string TraCICommandInterface::Pedestrian::getTypeId() {
...
void TraCICommandInterface::Pedestrian::setColor(const TraCIColor& color) {
    ...
    p << nodeId;
    p << static_cast<uint8_t>(TYPE_COLOR) << color.red << color.green << color.blue << color.alpha;
    TraCIBuffer buf = connection->query(CMD_SET_PERSON_VARIABLE, p);
    ASSERT(buf.eof());
}

void TraCICommandInterface::Pedestrian::setParameter(const std::string& parameter, int value)
{
...
void TraCICommandInterface::Pedestrian::setParameter(const std::string& parameter, double value)
{
    std::stringstream strValue;
    strValue << value;
    setParameter(parameter, strValue.str());
...
void TraCICommandInterface::Pedestrian::setParameter(const std::string& parameter, const std::string& value)
{
    static int32_t nParameters = 2;
    TraCIBuffer buf = traci->connection.query(CMD_SET_PERSON_VARIABLE, TraCIBuffer() << static_cast<uint8_t>(VAR_PARAMETER) << nodeId << static_cast<uint8_t>(TYPE_COMPOUND) << nParameters << static_cast<uint8_t>(TYPE_STRING) << parameter << static_cast<uint8_t>(TYPE_STRING) << value);
    ASSERT(buf.eof());
}

void TraCICommandInterface::Pedestrian::getParameter(const std::string& parameter, int& value)
{
    std::string v;
...
void TraCICommandInterface::Pedestrian::getParameter(const std::string& parameter, double& value)
{
    std::string v;
    getParameter(parameter, v);
    ParBuffer buf(v);
...
void TraCICommandInterface::Pedestrian::getParameter(const std::string& parameter, std::string& value)
{
    TraCIBuffer response = traci->connection.query(CMD_GET_PERSON_VARIABLE, TraCIBuffer() << static_cast<uint8_t>(VAR_PARAMETER) << nodeId << static_cast<uint8_t>(TYPE_STRING) << parameter);
    uint8_t cmdLength;
    response >> cmdLength;
    uint8_t responseId;
    response >> responseId;
    ASSERT(responseId == RESPONSE_GET_PERSON_VARIABLE);
    uint8_t variable;
    response >> variable;
