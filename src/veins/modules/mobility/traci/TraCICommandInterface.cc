...
bool TraCICommandInterface::addPed(std::string PedId, std::string PedTypeId, std::string routeId, simtime_t emitTime_st, double emitPosition, double emitSpeed, int8_t emitLane) {
    ...
    int32_t count = 6;
    int32_t emitTime = (emitTime_st < 0) ? (-1) : (floor(emitTime_st.dbl() * 1000));
    TraCIBuffer buf = connection.queryOptional(CMD_SET_PERSON_VARIABLE, TraCIBuffer() << variableId << PedId << variableType << count << (uint8_t)TYPE_STRING << PedTypeId << (uint8_t)TYPE_STRING << routeId << (uint8_t)TYPE_INTEGER << emitTime << (uint8_t)TYPE_DOUBLE << emitPosition << (uint8_t)TYPE_DOUBLE <<emitSpeed << (uint8_t)TYPE_BYTE << emitLane, success);
    ASSERT(buf.eof());
