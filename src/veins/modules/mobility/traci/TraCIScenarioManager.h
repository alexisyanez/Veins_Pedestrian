...
class TraCIScenarioManager : public cSimpleModule
{
	...
	protected:
        ...
        //maps from vehicle type to moduleType, moduleName, and moduleDisplayString
        typedef std::map<std::string, std::string> PedTypeMapping;
		uint32_t PedNameCounter;
		std::vector<std::string> pedTypeIds;
		std::map<int, std::queue<std::string> > pedInsertQueue;
		std::set<std::string> queuedPed;
		int pedRngIndex;
		int numPed;

        PedTypeMapping PedmoduleType; /**< module type to be used in the simulation for each managed vehicle */
        PedTypeMapping PedmoduleName; /**< module name to be used in the simulation for each managed vehicle */
        PedTypeMapping PedmoduleDisplayString; /**< module displayString to be used in the simulation for each managed vehicle */
		...
		std::set<std::string> subscribedPed; /**< all vehicles we have already subscribed to */
		uint32_t activePedCount; /**< number of vehicles, be it parking or driving **/
		size_t nextPedNodeVectorIndex;
		std::vector<std::string> subscribedPedObject;
		/**
        ...
        /**
         * tries to add all person in the vehicle queue to SUMO;
         */
        void insertPerson();
		...
		void processPersonSubscription(std::string objectId, TraCIBuffer& buf); //Code by Alexis Yáñez

		void processSubcriptionResult(TraCIBuffer& buf);

		void addPedModule(std::string nodeId, std::string type, std::string name, std::string displayString, const Coord& position, std::string road_id = "", double speed = -1, double angle = -1);
		virtual void updateModulePositionPed(cModule* mod, const Coord& p, const std::string& edge, double speed, double angle);

		virtual void preInitializePedModule(cModule* mod, const std::string& nodeId, const Coord& position, const std::string& road_id, double speed, double angle);
		...
		PedTypeMapping PedparseMappings(std::string parameter, std::string parameterName, bool allowEmpty = false);
